...
public class Ex01_Casting {
	public static void main(String[] args) {
		...
		// 이런 변수의 타입을 용도에 따라서 바꿀수 있는데 이걸 우리는 타입변환 이라고 표현한다.
		// 타입변환은 크게 두가지가 있음.
		// 1.자동 타입 변환(묵시적 형변환, UpCasting)
		// 작은 타입이 (byte) 가 큰 타입 (int) 로 자동으로 변환 되는 것을 의미.
		// 뭔가 따로 코딩을 안해도 되기 때문에 묵시적이라는 표현도 사용함.
		...
		// 큰 타입 (int) 가 작은 타입(short,byte)으로 바꿔줘야 할 때 하는 형변환을 의미함.
